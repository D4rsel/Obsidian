...
```shell-session
D4rsel@htb[/htb]$ git clone https://github.com/jpillora/chisel.git
```
...
```shell-session
D4rsel@htb[/htb]$ scp chisel ubuntu@10.129.202.64:~/
...
```shell-session
D4rsel@htb[/htb]$ ./chisel client -v 10.129.202.64:1234 socks
...
```shell-session
D4rsel@htb[/htb]$ tail -f /etc/proxychains.conf 
...
```shell-session
D4rsel@htb[/htb]$ sudo ./chisel server --reverse -v -p 1234 --socks5
